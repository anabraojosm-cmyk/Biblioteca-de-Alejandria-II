...
const iconBtn: React.CSSProperties = { display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 4, padding: "5px 7px", borderStyle: "solid", borderWidth: 1, borderColor: "var(--border)", borderRadius: 6, background: "var(--bg2)", color: "var(--text)", cursor: "pointer" };
...
const EditorContent = memo(function EditorContent({
  active, activeId, editorRef, handleInput, pushHistory,
}: any) {
  const shellRef = useRef<HTMLDivElement>(null);
  const callbacksRef = useRef({ handleInput, pushHistory });
  callbacksRef.current = { handleInput, pushHistory };
  ...
  useEffect(() => {
    ...
    editor.contentEditable = String(!active.readOnly);
    editor.className = active.dropCap ? "dropcap-editor" : "";

    // Evita que extensiones del navegador (Grammarly, correctores,
    // traductores automáticos, etc.) manipulen el DOM del contentEditable
    // y provoquen errores de React al re-renderizar.
    ...
    Object.assign(editor.style, {
      ...
      padding: "22px",
      color: "var(--text)",
      fontFamily: `'${active.font}', serif`,
      fontSize: "16.5px",
      lineHeight: "1.8",
      outline: "none",
      textAlign: active.justify ? "justify" : "left",
      overflowY: "auto",
      textIndent: active.indent ? "2em" : "0",
      opacity: active.readOnly ? "0.75" : "1",
    });
    editor.innerHTML = active.content;
    shell.appendChild(editor);
    editorRef.current = editor;

    const onInput = () => callbacksRef.current.handleInput(true);
    const onBeforeInput = () => callbacksRef.current.pushHistory();
    ...
    return () => {
      ...
      if (editor.parentNode === shell) shell.removeChild(editor);
  ...
  useEffect(() => {
    ...
    editor.contentEditable = String(!active.readOnly);
    editor.className = active.dropCap ? "dropcap-editor" : "";
    editor.style.setProperty("--editor-font", `'${active.font}', serif`);
    editor.style.fontFamily = `'${active.font}', serif`;
    editor.style.textAlign = active.justify ? "justify" : "left";
    editor.style.textIndent = active.indent ? "2em" : "0";
    editor.style.opacity = active.readOnly ? "0.75" : "1";
  }, [active.readOnly, active.dropCap, active.font, active.justify, active.indent]);
  ...
  && previous.active.readOnly === next.active.readOnly
  && previous.active.dropCap === next.active.dropCap
  && previous.active.font === next.active.font
  && previous.active.justify === next.active.justify
  && previous.active.indent === next.active.indent
));
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: isMobile ? "column" : "row", gap: 16, minWidth: 0 }}>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        ...
        <EditorContent active={active} activeId={activeId} editorRef={editorRef} handleInput={handleInput} pushHistory={pushHistory} />
