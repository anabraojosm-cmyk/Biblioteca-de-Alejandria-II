...
export function ChaptersTab({ bookId, chapters, setChapters, bookActs, setBookActs, isMobile }: any) {
  ...
  useEffect(() => {
    if (editorRef.current && active) { editorRef.current.innerHTML = active.content; setWc(wordCount(active.content)); }
  ...
  function handleInput(recordHistory = true) {
    ...
    setDirty(true);
    if (editorRef.current) setWc(wordCount(editorRef.current.innerHTML));
    clearTimeout(autosaveTimer.current);
